...
const editTestSchema = z.object({
  ...
  ranges: z.array(rangeSchema).min(1, "Додайте хоча б один діапазон результату"),
});
...
export async function updateTestAction(
  testId: string,
  _prevState: EditTestState,
  formData: FormData
): Promise<EditTestState> {
  await requireAdmin();
  ...
  const parsed = editTestSchema.safeParse(payload);
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.testScaleResult.deleteMany({
      where: {
        scale: {
          testId,
        },
